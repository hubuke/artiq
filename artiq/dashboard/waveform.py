...
class WaveformDock(QtWidgets.QDockWidget):
    ...
    def _update_uniform_interval(self):
        ...
        self._channel_model.update(channel_list)
        if self._uniform_interval:
            self._waveform_view._top.setScale(1e0)
            self._waveform_view._top.setLabel(text= "event index", units="events")
        else:
            self._waveform_view._top.setScale(1e-12)
            self._waveform_view._top.setLabel(text= "time", units="s")
