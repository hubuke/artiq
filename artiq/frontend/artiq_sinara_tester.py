...
class SinaraTester(EnvExperiment):
    ...
    def setup_phaser_servo(self, phaser, duc):
        ...
        phaser.channel[0].set_servo(profile=0, enable=0, hold=1)
        phaser.channel[1].set_servo(profile=0, enable=0, hold=1)
        delay(1*ms)
        phaser.channel[0].set_iir(profile=0, kp=-1., ki=0., g=0., x_offset=-0.1, y_offset=0.0)
        phaser.channel[1].set_iir(profile=0, kp=-1., ki=0., g=0., x_offset=-0.1, y_offset=0.0)
        delay(1*ms)
